...
// Sur Render, on utilise une URL de connexion fournie en variable d'environnement
// Exemple Postgres : postgres://user:pass@host:5432/dbname
// Exemple MySQL externe (PlanetScale/Aiven) : mysql://user:pass@host:3306/dbname
$databaseUrl = getenv('DATABASE_URL');
 
if (!$databaseUrl) {
    echo json_encode(["status" => "error", "message" => "Variable DATABASE_URL non définie."]);
    exit;
}
...
if ($parts === false || !isset($parts['scheme'], $parts['host'], $parts['path'])) {
    echo json_encode(["status" => "error", "message" => "DATABASE_URL invalide.", "debug" => $databaseUrl]);
    exit;
}
 
$driver   = ($parts['scheme'] === 'mysql') ? 'mysql' : 'pgsql';
$host     = $parts['host'];
